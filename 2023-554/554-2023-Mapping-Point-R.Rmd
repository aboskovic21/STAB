...
library(knitr)
opts_chunk$set(collapse=TRUE, fig.align='center', warning=FALSE,message=FALSE,cache=FALSE)
```
...
```{r}
beta_0 <- lmfit$coefficients[1] # intercept
beta_1 <- lmfit$coefficients[2] # distance
beta_2 <- lmfit$coefficients[3] # elevation

small_grid <- cbind(meuse$x, meuse$y) %>% as.data.frame()
kc_small <- krige.conv(detrend,loc=small_grid,
krige=krige.control(obj.m=mlfit2))

pred <- beta_0 + beta_1 * meuse$dist + beta_2 * meuse$elev + kc_small$predict

image.plot(x=sort(unique(small_grid[["V1"]])[1:144]),
           y=sort(unique(small_grid[["V2"]])[1:144]),
           z=matrix(pred[1:144],nrow=12,ncol=12),col=terrain.colors(100),xlab="x-coordinate",ylab="y-coordinate")
symbols(detrend$coords[,1],detrend$coords[,2],
  circles=(detrend$data-min(detrend$data))/1,add=T,inches=0.04)
```
