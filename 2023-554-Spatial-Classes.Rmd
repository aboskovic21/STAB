...
We can also do this using `ggplot()`, which is part of the `tidyverse` package, as shown below:
...
We zoom in on King County (using the `plot()` function from base R):
...
Again, we can do this using `ggplot()`, too:
