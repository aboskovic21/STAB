...
library(maptools) # for leglabs: note that maptools will disappear from CRAN June 2023
...
```{r}
library(ggplot2)
ggplot() + geom_sf(data = st_set_crs(wacounty, 4326), aes(fill = TotPop90))
```


As an alternative we can use the ``plot`` function, which uses base plot for spatial data with attributes. We map the 1990 Washington population counts by county.
...
# Add some labels
text(st_coordinates(st_centroid(wacounty)[xx, ]), "King", cex=0.75, pos=3, offset=0.25)
text(st_coordinates(st_centroid(wacounty)[xx2, ]), "Spokane", cex=0.7, pos=1, offset=0.25)
points(st_coordinates(st_centroid(wacounty)[c(xx, xx2), ]), pch=16, cex=0.75)
```
...
plot(st_geometry(nc))
IDs = substr(nc$ID, 16, 100)
text(centroids[,1], centroids[,2], IDs, offset=0, cex=0.4)
```
