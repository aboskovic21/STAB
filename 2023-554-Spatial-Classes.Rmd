...
We first create a ``st_multilinestring`` object containing the coordinates of the streets: 

```{r}
# Streets
slist <- split(Snow.streets[,c("x","y")],
               as.factor(Snow.streets[,"street"]))
slist <- lapply(slist, as.matrix)
m <- st_multilinestring(slist)
plot(m)
```
...
plot(m, col="gray")
# deaths
p <- st_as_sf(Snow.deaths[,c("x","y")], coords = c("x", "y"))
plot(p, add=TRUE, col ='red', pch=15, cex=0.6)
# pumps
spp <- st_as_sf(Snow.pumps[,c("x","y")], coords = c("x", "y"))
...
## Adapted from maptools::leglabs()
make_legend <- function (vec, under = "under", over = "over", between = "-") 
{
  x <- vec
  lx <- length(x)
  res <- character(lx - 1)
...
legend("bottomright", legend=make_legend(round(brks, digits = 1)),
       fill=plotclr,cex=0.35,bty="n")
```
...
ggplot(data = wacounty) +
  ...
  scale_fill_manual(values = rev(plotclr), labels = rev(make_legend(round(brks,digits=1)))) +
  theme_bw() +
  coord_sf() +
  xlab("") +
  ylab("") +
...
# plot the whole sate
plot(wacounty$geometry)

# highlight counties of interest
plot(wacounty[xx, ]$geometry, col="#ff000075", add=T)
plot(wacounty[xx2, ]$geometry, col=NA, border="#0000ff75", add=T, lwd=2.5)

# Add some labels
text(sf::st_coordinates(sf::st_centroid(wacounty[xx,]$geometry)),
     "King", cex=0.75, pos=3, offset=0.25, col = "red")
text(sf::st_coordinates(sf::st_centroid(wacounty[xx2,]$geometry)),
     "Spokane", cex=0.75, pos=3, offset=0.25, col = "blue")
points(sf::st_coordinates(sf::st_centroid(wacounty[c(xx, xx2),]$geometry)),
       cex=0.75, pch=16)
```
...
plot(nc["BIR74"]$geom)
text(sf::st_coordinates(st_centroid(nc["BIR74"]$geom)), rownames(nc), cex=0.4, offset=0)
```

In ggplot:

```{r}
centroids <- sf::st_coordinates(st_centroid(nc["BIR74"]$geom))
